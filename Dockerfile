...
# Gnirehtet only has a pre-built x86_64 binary; skip on other architectures.
# REVERSE_TETHER will log a warning and be a no-op on unsupported arches.
...
RUN set -ex && \
    ...
    if [ "$ARCH" = "x86_64" ]; then \
        ...
        echo "[*] Skipping gnirehtet (no binary for $ARCH)"; \
    fi
